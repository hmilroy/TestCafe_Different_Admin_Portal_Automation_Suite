...
class addLeaseComponent extends BaseComponent {
  ...
  async execute(data) {
    const frequencySelector = Selector(
      "div.content-dock div.row.dashboard div.col.dashboard-content-area:nth-child(2) div.admin-padding-adjustment.add-person-form.people-tab div.leases-section:nth-child(3) div:nth-child(2) div.lease-row.row:nth-child(3) div.col-md-3:nth-child(2) > select:nth-child(1)"
    );
    const frequencyOptions = frequencySelector.find("option");

    const secondCalender = Selector(() => {
      return document.querySelectorAll(
        "input[class='ant-calendar-picker-input ant-input']"
      )[2];
    });
